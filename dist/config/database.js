...
async function initializeDatabase() {
    try {
        ...
        pool.on('error', (err) => {
            logging_1.logger.error({ err }, 'Unexpected error on idle database client');
        });
        // Test connection
        const client = await pool.connect();
        await client.query('SELECT NOW()');
        client.release();
        logging_1.logger.info('✓ Connected to PostgreSQL');
    }
    catch (error) {
        logging_1.logger.warn('PostgreSQL database connection failed — falling back to in-memory Mock Database mode');
        useMockDb = true;
    }
}
exports.db = {
    ...
    transaction: async (callback) => {
        if (useMockDb) {
            const mockClient = {
                query: (text, params) => Promise.resolve(simulateQuery(text, params)),
            };
